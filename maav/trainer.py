...
class Trainer:
    ...
    def train_model(self, epochs=10, batch_size=32, save=False, process_dataset=False):
        ...
        print("\nStarting training.")
        try:
            history = self.model.train_model(tf.cast(tf.constant(self.dataset.X_train), tf.int32), tf.cast(tf.constant(self.dataset.y_train), tf.int8), epochs=epochs, batch_size=batch_size)
