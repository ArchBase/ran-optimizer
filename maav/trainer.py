...
class Trainer:
    ...
    def train_model_using_adam(self, epochs=10, batch_size=32, save=False, process_dataset=False):
        ...
        try:
            ...
            self.model.train_model_using_adam(tf.cast(tf.constant(self.dataset.X_train), tf.int32), tf.cast(tf.constant(self.dataset.y_train), tf.int32), epochs=epochs, batch_size=batch_size)
            end_time = time.time()
